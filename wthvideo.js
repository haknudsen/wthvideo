...
function wthplayer() {
	...
	function isDevice() {
		...
		if (ua.indexOf("android") > -1) {
			return true;
		} else if (ua.includes("iphone os 10")) {
			return false;
		} else if (platform.indexOf("iP")> - 1) {
			return true;
		} else{return false;}
	}
...
(function () {
	function c(a, d, b) {
		...
		this.removed = this.immediatePropagationStopped = this.propagationStopped = this.defaultPrevented = false
	}
	var b = c.prototype;
...
(function () {
	...
	b.willTrigger = function (a) {
		for (var d = this; d;) {
			if (d.hasEventListener(a)) return true;
			d = d.parent
		}
		return false
	};
	...
	b._dispatchEvent = function (a, d) {
		...
		if (a && c && (c = c[a.type]) && (b = c.length)) {
			...
			for (var c = c.slice(), f = 0; b > f && !a.immediatePropagationStopped; f++) {
				...
				a.removed && (this.off(a.type, g, 1 == d), a.removed = false)
			}
		}
	};
...
(function () {
	function c(d, b) {
		function h(a) {
			if (h[a] !== u) return h[a];
			var d;
			if ("bug-string-char-index" == a) d = false;
					...
					z = function () {
						for (var a, d, b, c, e, h = G, f = h.length; f > l;) switch (e = h.charCodeAt(l)) {
							...
							default:
								...
								if (c && n(), "true" == h.slice(l, l + 4)) return l += 4, true;
								if ("false" == h.slice(l, l + 5)) return l += 5, false;
								if ("null" == h.slice(l,
										l + 4)) return l += 4, null;
								n()
						}
...
(function () {
	createjs.DataUtils = {
		parseXML: function (c, b) {
			...
			if (!a) try {
				a = new ActiveXObject("Microsoft.XMLDOM"), a.async = false, a.loadXML(c)
			} catch (d) {
				a = null
			}
			return a
...
(function () {
	var c = {
		...
		isAudioTag: function (b) {
			return window.HTMLAudioElement ? b instanceof HTMLAudioElement : false
		},
		isVideoTag: function (b) {
			return window.HTMLVideoElement ? b instanceof HTMLVideoElement : false
		},
		isText: function (b) {
			switch (b) {
...
(function () {
	function c(a, d, b) {
		...
		this.canceled = this.loaded = false;
		this.progress = 0;
		this.type = b;
...
(function () {
	...
	b.load = function () {
		...
		null != this._request.addEventListener ? (this._request.addEventListener("loadstart", this._handleLoadStartProxy, false), this._request.addEventListener("progress", this._handleProgressProxy, false), this._request.addEventListener("abort", this._handleAbortProxy, false), this._request.addEventListener("error", this._handleErrorProxy, false), this._request.addEventListener("timeout", this._handleTimeoutProxy, false), this._request.addEventListener("load", this._handleLoadProxy, false), this._request.addEventListener("readystatechange",
	...
	b._createXHR = function (a) {
		...
		f = null;
		if (f = a.method == createjs.AbstractLoader.GET ? createjs.RequestUtils.buildPath(a.src, a.values) : a.src, c.open(a.method || createjs.AbstractLoader.GET, f, true), d && c instanceof XMLHttpRequest && 1 == this._xhrLevel &&
		...
		return c instanceof XMLHttpRequest && void 0 !== a.withCredentials && (c.withCredentials = a.withCredentials), this._request = c, true
	};
	b._clean = function () {
		clearTimeout(this._loadTimeout);
...
(function () {
	...
	b.loadFile = function (a, d, b) {
		...
		this.setPaused(false !== d ? false : true)
	};
	...
	b.getItems = function (a) {
		...
		for (b in this._loadItemsById) {
			var c = this._loadItemsById[b],
				f = this.getResult(b);
			true === a && null == f || d.push({
				item: c,
				result: f,
				rawResult: this.getResult(b, true)
			})
		}
		return d
	};
	...
	b._addItem = function (a, d, b) {
		...
		null != a && (d = this._createLoader(a), null != d && ("plugins" in d && (d.plugins = this._plugins), a._loader = d, this._loadQueue.push(d), this._loadQueueBackup.push(d), this._numItems++, this._updateProgress(), (this.maintainScriptOrder && a.type == createjs.LoadQueue.JAVASCRIPT || true === a.maintainOrder) && (this._scriptOrder.push(a), this._loadedScripts.push(null))))
	};
	b._createLoadItem = function (a, d, b) {
		...
		if (d = this._typeCallbacks[a.type] || this._extensionCallbacks[a.ext]) {
			...
			if (false === d) return null;
			true === d || null != d && (a._loader = d);
			e = createjs.RequestUtils.parseURI(a.src);
	...
	b._loadNext = function () {
		if (!this._paused) {
			...
			this._numItems == this._numItemsLoaded ? (this.loaded = true, this._sendComplete(), this.next && this.next.load && this.next.load()) : this.loaded = false;
	...
	b._handleError = function (a) {
		...
		this._finishOrderedItem(a, true);
		this._updateProgress();
		var d = new createjs.ErrorEvent("FILE_LOAD_ERROR", null, a.getItem());
		this._sendError(d);
		this.stopOnError ? this.setPaused(true) : (this._removeLoadItem(a), this._cleanLoadItem(a), this._loadNext())
	};
	...
	b._finishOrderedItem = function (a, d) {
		...
		if (this.maintainScriptOrder && b.type == createjs.LoadQueue.JAVASCRIPT || b.maintainOrder) {
			a instanceof createjs.JavaScriptLoader && (this._currentlyLoadingScript = false);
			var c = createjs.indexOf(this._scriptOrder, b);
			return -1 == c ? false : (this._loadedScripts[c] = true === d ? true : b, this._checkScriptLoadOrder(), true)
		}
		return false
	};
	...
	b._canStartLoad = function (a) {
		if (!this.maintainScriptOrder || a.preferXHR) return true;
		a = a.getItem();
		if (a.type != createjs.LoadQueue.JAVASCRIPT) return true;
		if (this._currentlyLoadingScript) return false;
		a = this._scriptOrder.indexOf(a);
		for (var d = 0; a > d;) {
			if (null == this._loadedScripts[d]) return false;
			d++
		}
...
(function () {
	function c(b) {
		this.AbstractLoader_constructor(b, true, createjs.AbstractLoader.TEXT)
...
(function () {
	function c(a) {
		this.AbstractLoader_constructor(a, true, createjs.AbstractLoader.BINARY);
		this.on("initialize", this._updateXHR, this)
	}
	var b = createjs.extend(c, createjs.AbstractLoader);
...
(function () {
	...
	b._formatResult = function (a) {
		if (this._preferXHR) {
			var d = a.getTag();
			d.styleSheet ? d.styleSheet.cssText = a.getResult(true) : (a =
				document.createTextNode(a.getResult(true)), d.appendChild(a))
		} else d = this._tag;
...
(function () {
	function c(a, d) {
		...
		null != this._tag ? this._preferXHR = false : this._tag = document.createElement("img");
		this.on("initialize", this._updateXHR, this)
	}
	var b = createjs.extend(c, createjs.AbstractLoader);
	c.canLoadItem =
	...
	b._formatImage = function (a, d) {
		...
		this._preferXHR && (c ? (c = c.createObjectURL(this.getResult(true)), b.src = c, b.addEventListener("load", this._cleanUpURL, false), b.addEventListener("error", this._cleanUpURL, false)) : b.src = this._item.src);
...
(function () {
	...
	b._formatResult = function (a) {
		var d = a.getTag();
		return this._preferXHR && (d.text = a.getResult(true)), d
	};
...
(function () {
	function c(a) {
		this.AbstractLoader_constructor(a, true, createjs.AbstractLoader.JSON);
		this.resultFormatter = this._formatResult
	}
	var b = createjs.extend(c, createjs.AbstractLoader);
	c.canLoadItem = function (a) {
	...
	b._formatResult = function (a) {
		var d = null;
		try {
			d = createjs.DataUtils.parseJSON(a.getResult(true))
		} catch (e) {
...
(function () {
	function c(a) {
		this.AbstractLoader_constructor(a, false, createjs.AbstractLoader.JSONP);
		this.setTag(document.createElement("script"));
...
(function () {
		...
		switch (a.type) {
			case "complete":
				return this._rawResult =
					a.target.getResult(true), this._result = a.target.getResult(), this._sendProgress(c.MANIFEST_PROGRESS), void this._loadManifest(this._result);
			case "progress":
	...
	b._loadManifest = function (a) {
		if (a && a.manifest) {
			...
			d.on("complete", this._handleManifestComplete, this, true);
			d.on("error", this._handleManifestError, this, true);
...
(function () {
	function c(a, d) {
		...
		null != this._tag && (this._preferXHR = false)
	}
	...
	b._createTag = function (a) {
		...
		return d.autoplay = false, d.preload = "none", d.src = a, d
	};
...
(function () {
	function c(b, a) {
		...
		createjs.RequestUtils.isVideoTag(b) || createjs.RequestUtils.isVideoTag(b.src) ? (this.setTag(createjs.RequestUtils.isVideoTag(b) ? b : b.src), this._preferXHR = false) : this.setTag(this._createTag())
	}
...
(function () {
	...
	b.handleEvent =
		function (a) {
			switch (a.type) {
				case "complete":
					return this._rawResult = a.target.getResult(true), this._result = a.target.getResult(), this._sendProgress(c.SPRITESHEET_PROGRESS), void this._loadManifest(this._result);
				case "progress":
	...
	b._handleManifestComplete = function () {
		...
		this._loadedItems =
			this._manifestQueue.getItems(true);
		this._sendComplete()
	};
	b._handleManifestProgress = function (a) {
...
(function () {
	...
	b._formatResult = function (a) {
		var d = createjs.DataUtils.parseXML(a.getResult(true), "text/xml");
		a =
			a.getTag();
...
(function () {
	function c(a) {
		this.AbstractLoader_constructor(a, true, createjs.AbstractLoader.XML);
		this.resultFormatter = this._formatResult
	}
	var b = createjs.extend(c, createjs.AbstractLoader);
	c.canLoadItem = function (a) {
	...
	b._formatResult = function (a) {
		return createjs.DataUtils.parseXML(a.getResult(true), "text/xml")
	};
...
(function () {
	...
	c._handleLoadError = function (a) {
		...
		if (c._preloadHash[b])
			for (var d = 0, f = c._preloadHash[b].length; f > d; d++) {
				var g = c._preloadHash[b][d];
				if (c._preloadHash[b][d] = false, c.hasEventListener("fileerror")) a = new createjs.Event("fileerror"), a.src = g.src, a.id = g.id, a.data = g.data, a.sprite = g.sprite, c.dispatchEvent(a)
			}
	};
	c._registerPlugin = function (a) {
		return a.isSupported() ? (c.activePlugin = new a, true) : false
	};
	c.registerPlugins = function (a) {
		c._pluginsRegistered = true;
		for (var b = 0, d = a.length; d > b; b++)
			if (c._registerPlugin(a[b])) return true;
		return false
	};
	c.initializeDefaultPlugins = function () {
		return null != c.activePlugin ? true : c._pluginsRegistered ? false : c.registerPlugins([createjs.WebAudioPlugin, createjs.HTMLAudioPlugin]) ? true : false
	};
	c.isReady = function () {
	...
	c._registerSound = function (a) {
		if (!c.initializeDefaultPlugins()) return false;
		var d;
		if (a.src instanceof Object ? (d = c._parseSrc(a.src), d.src = a.path + d.src) : d = c._parsePath(a.src), null == d) return false;
		a.src = d.src;
		a.type = "sound";
		d = a.data;
		var h = null;
	...
	c.registerSound = function (a, b, h, f, g) {
		...
		else if (1 == c._preloadHash[h.src][0]) return true;
		return h
	};
	...
	c.removeSound = function (a, e) {
		if (null == c.activePlugin) return false;
		a instanceof Object && a.src && (a = a.src);
		var d;
		if (a instanceof Object ? d = c._parseSrc(a) : (a = c._getSrcById(a).src, d = c._parsePath(a)), null == d) return false;
		a = d.src;
		null != e && (a = e + a);
		for (var f in c._idHash) c._idHash[f].src == a && delete c._idHash[f];
		return b.removeSrc(a), delete c._preloadHash[a], c.activePlugin.removeSound(a), true
	};
	c.removeSounds =
		function (a, b) {
			var d = [];
	...
	c.loadComplete = function (a) {
		...
		return a = b ? c._getSrcById(b.src).src : c._getSrcById(a).src, void 0 == c._preloadHash[a] ? false : 1 == c._preloadHash[a][0]
	};
	...
	c._playInstance = function (a, b) {
		...
		if (null == b.interrupt && (b.interrupt = d.interrupt || c.defaultInterruptBehavior), null == b.delay && (b.delay = d.delay || 0), null == b.offset && (b.offset = a.getPosition()), null == b.loop && (b.loop = a.loop), null == b.volume && (b.volume = a.volume), null == b.pan && (b.pan = a.pan), 0 == b.delay) {
			if (!c._beginPlaying(a, b)) return false
		} else d = setTimeout(function () {
			c._beginPlaying(a,
				b)
		}, b.delay), a.delayTimeoutId = d;
		return this._instances.push(a), true
	};
	c._beginPlaying = function (a, c) {
		if (!b.add(a, c.interrupt)) return false;
		if (!a._beginPlaying(c)) {
			var d = createjs.indexOf(this._instances, a);
			return -1 < d && this._instances.splice(d, 1), false
		}
		return true
	};
	...
	b.create = function (a, c) {
		return null == b.get(a) ? (b.channels[a] =
			new b(a, c), true) : false
	};
	b.removeSrc = function (a) {
		var d = b.get(a);
		return null == d ? false : (d._removeAll(), delete b.channels[a], true)
	};
	b.removeAll = function () {
	...
	b.add = function (a, c) {
		var d = b.get(a.src);
		return null == d ? false : d._add(a, c)
	};
	b.remove = function (a) {
		var d = b.get(a.src);
		return null == d ? false : (d._remove(a), true)
	};
	...
	a._add = function (a, b) {
		return this._getSlot(b, a) ? (this._instances.push(a), this.length++, true) : false
	};
	a._remove = function (a) {
		a = createjs.indexOf(this._instances, a);
		return -1 == a ? false : (this._instances.splice(a, 1), this.length--, true)
	};
	...
	a._getSlot = function (a) {
		var b,
			d;
		if (a != c.INTERRUPT_NONE && (d = this._get(0), null == d)) return true;
		for (var f = 0, g = this.max; g > f; f++) {
			if (b = this._get(f), null == b) return true;
		...
		return null != d ? (d._interrupt(), this._remove(d), true) : false
	};
	a.toString = function () {
...
(function () {
	var c = function (a, b, c, h) {
			...
			false !== h && true !== h && this.setPlaybackResource(h);
			this._position = 0;
	...
	b.play = function (a, b, c, h, f, g) {
		...
		return d = createjs.PlayPropsConfig.create(a instanceof Object || a instanceof createjs.PlayPropsConfig ? a : {
			...
			volume: f,
			pan: g
		}), this.playState == createjs.Sound.PLAY_SUCCEEDED ? (this.applyPlayProps(d), void(this._paused && this.setPaused(false))) : (this._cleanUp(), createjs.Sound._playInstance(this, d), this)
	};
	b.stop = function () {
		return this._position = 0,
			this._paused = false, this._handleStop(), this._cleanUp(), this.playState = createjs.Sound.PLAY_FINISHED, this
	};
	b.destroy = function () {
		this._cleanUp();
	...
	b.setPaused = function (a) {
		return true !== a && false !== a || this._paused == a || 1 == a && this.playState != createjs.Sound.PLAY_SUCCEEDED ? void 0 : (this._paused = a, a ? this._pause() : this._resume(), clearTimeout(this.delayTimeoutId), this)
	};
	...
	b.setMuted = function (a) {
		return true === a || false === a ? (this._muted = a, this._updateVolume(),
			this) : void 0
	};
	b.getMuted = function () {
		return this._muted
	...
	b._beginPlaying = function (a) {
		return this.setPosition(a.offset), this.setLoop(a.loop), this.setVolume(a.volume), this.setPan(a.pan), null != a.startTime && (this.setStartTime(a.startTime), this.setDuration(a.duration)), null != this._playbackResource && this._position < this._duration ? (this._paused = false, this._handleSoundReady(), this.playState = createjs.Sound.PLAY_SUCCEEDED, this._sendEvent("succeeded"), true) : (this._playFailed(), false)
	};
...
(function () {
	...
	b.setVolume = function (a) {
		return this._volume = a, this._updateVolume(), true
	};
	b.getVolume = function () {
		return this._volume
	};
...
(function () {
	function c(a) {
		this.AbstractLoader_constructor(a, true, createjs.AbstractLoader.SOUND)
	}
	var b = createjs.extend(c, createjs.AbstractLoader);
	c.context = null;
	b.toString = function () {
...
(function () {
	...
	a.isSupported = function () {
		...
		return "file:" != location.protocol || b || this._isFileXHRSupported() ? (a._generateCapabilities(), null == a.context ? false : true) : false
	};
	...
	a._isFileXHRSupported = function () {
		...
		try {
			b.open("GET", "WebAudioPluginTest.fail", false)
		} catch (h) {
			return a = false
		}
		b.onerror = function () {
	...
	a._generateCapabilities = function () {
		if (null == a._capabilities) {
			...
			"ontouchstart" in window && "running" != a.context.state && (a._unlock(), document.addEventListener("mousedown", a._unlock, true), document.addEventListener("touchend", a._unlock, true));
			a._capabilities = {
				panning: true,
				volume: true,
				tracks: -1
			...
			2 > a.context.destination.numberOfChannels && (a._capabilities.panning = false)
		}
	};
	...
	a._unlock = function () {
		a._unlocked || (a.playEmptySound(), "running" == a.context.state && (document.removeEventListener("mousedown", a._unlock, true), document.removeEventListener("touchend", a._unlock, true), a._unlocked = true))
	};
	b.toString = function () {
...
(function () {
	...
	c.set = function (a, b) {
		b == c._tags[a] ? c._tagUsed[a] = false : c._tagPool.set(b)
	};
	c.remove = function (a) {
		var b = c._tags[a];
		return null == b ? false : (c._tagPool.set(b), delete c._tags[a], delete c._tagUsed[a], true)
	};
...
(function () {
	...
	b._removeLooping = function () {
		null != this._playbackResource && (this._playbackResource.loop = false, this._playbackResource.removeEventListener(createjs.HTMLAudioPlugin._AUDIO_SEEKED, this._loopHandler, false))
	};
	b._addLooping = function () {
		null == this._playbackResource ||
			this._audioSpriteStopTime || (this._playbackResource.addEventListener(createjs.HTMLAudioPlugin._AUDIO_SEEKED, this._loopHandler, false), this._playbackResource.loop = true)
	};
	b._handleCleanUp = function () {
		...
		if (null != a) {
			a.pause();
			a.loop = false;
			a.removeEventListener(createjs.HTMLAudioPlugin._AUDIO_ENDED, this._endedHandler, false);
			a.removeEventListener(createjs.HTMLAudioPlugin._AUDIO_READY, this._readyHandler, false);
			a.removeEventListener(createjs.HTMLAudioPlugin._AUDIO_STALLED, this._stalledHandler, false);
			a.removeEventListener(createjs.HTMLAudioPlugin._AUDIO_SEEKED, this._loopHandler, false);
			a.removeEventListener(createjs.HTMLAudioPlugin._TIME_UPDATE, this._audioSpriteEndHandler, false);
			try {
	...
	b._handleSoundReady = function () {
		if (4 !== this._playbackResource.readyState) {
			...
			return a.addEventListener(createjs.HTMLAudioPlugin._AUDIO_READY, this._readyHandler, false), a.addEventListener(createjs.HTMLAudioPlugin._AUDIO_STALLED, this._stalledHandler, false), a.preload = "auto", void a.load()
		}
		this._updateVolume();
		this._playbackResource.currentTime = .001 * (this._startTime + this._position);
		this._audioSpriteStopTime ? this._playbackResource.addEventListener(createjs.HTMLAudioPlugin._TIME_UPDATE, this._audioSpriteEndHandler, false) : (this._playbackResource.addEventListener(createjs.HTMLAudioPlugin._AUDIO_ENDED,
			this._endedHandler, false), 0 != this._loop && (this._playbackResource.addEventListener(createjs.HTMLAudioPlugin._AUDIO_SEEKED, this._loopHandler, false), this._playbackResource.loop = true));
	...
	b._handleSetPositionSeek = function () {
		null != this._playbackResource && (this._playbackResource.removeEventListener(createjs.HTMLAudioPlugin._AUDIO_SEEKED, this._handleSetPositionSeek, false), this._playbackResource.addEventListener(createjs.HTMLAudioPlugin._AUDIO_SEEKED, this._loopHandler, false))
	};
	...
	b._handleLoop = function () {
		0 == this._loop && (this._playbackResource.loop = false, this._playbackResource.removeEventListener(createjs.HTMLAudioPlugin._AUDIO_SEEKED, this._loopHandler, false))
	};
	b._updateStartTime = function () {
		this._audioSpriteStopTime = .001 * (this._startTime + this._duration);
		this.playState == createjs.Sound.PLAY_SUCCEEDED && (this._playbackResource.removeEventListener(createjs.HTMLAudioPlugin._AUDIO_ENDED, this._endedHandler, false), this._playbackResource.addEventListener(createjs.HTMLAudioPlugin._TIME_UPDATE, this._audioSpriteEndHandler, false))
	};
	b._updateDuration = function () {
		this._audioSpriteStopTime = .001 * (this._startTime + this._duration);
		this.playState == createjs.Sound.PLAY_SUCCEEDED &&
			(this._playbackResource.removeEventListener(createjs.HTMLAudioPlugin._AUDIO_ENDED, this._endedHandler, false), this._playbackResource.addEventListener(createjs.HTMLAudioPlugin._TIME_UPDATE, this._audioSpriteEndHandler, false))
	};
	b._setDurationFromSource = function () {
...
function Player(c) {
	...
	this.player.filesAlreadyLoaded = false;
	this.player.exitOnComplete = false;
	this.player.delay = 1E3 * this.json.delay;
	this.player.playMuted = false;
...
Player.prototype.setOptionsFromJson = function (c) {
	...
	sessionStorage.setItem(this.hasSeen, true);
	localStorage.setItem(this.hasSeen, true)
};
Player.prototype.preloadBtnClick = function () {
	this.player.filesAlreadyLoaded ? (this.player.playMuted = false, this.unmute(), this.repeat(), this.el.player.style.animationIterationCount = "", this.el.preloadControls.style.display = "none", this.el.playerControls.style.display = "block") : this.preload()
};
...
Player.prototype.start = function () {
	var c = 0 < arguments.length && void 0 !== arguments[0] ? arguments[0] : false;
	this.player.started = true;
	this.player.playing = true;
	this.player.stoped = false;
...
Player.prototype.resumeAudio = function () {
	this.player.audio.instance.paused = false
};
...
Player.prototype.unmute = function () {
	this.player.audio.instance.muted = false;
	this.player.muted = false;
	this.updateBtnMuteUnmuteStyle()
};
...
Player.prototype.onPreloadComplete = function () {
	...
	var c = window.URL.createObjectURL(this.preload.getResult("spriteSheet", true)),
		b = new Image;
	b.src = c;
	b.onload = function () {
